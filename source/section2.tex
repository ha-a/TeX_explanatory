...
\begin{itemize}
  ...
  文章の構造は\verb|part|, \verb|section|, \verb|subsection|, \verb|subsubsection|, \verb|paragraph|, \verb|subparagraph|の7つのレベルに分けられる．
  ...
  \item jsclasses (\verb|jsarticle| etc.): 日本語(u)\pLaTeX 用の新しい文書クラス\supercite{jscls_doc}．JISフォントメトリックを使用している．簡単に言えば「文字の組み方」が綺麗に，読みやすくなる．例えば小さい「ちょっとチャックをチェック」のように小さい文字を並べて表示する際\verb|jarticle|などのmin10フォントメトリックでは不自然に詰められ，美しくなかったり読みづらかったりするのがjsclassesでは解消される\supercite{LaTeX_are}．他にも日本語中の英語の組み方などに差が出る．
